...
class Document:
    ...
    def add_sent_id(self, events):
        ...
        for i, sent in enumerate(self.text):
            ...
            while e_id < len(events) and events[e_id]["position"][0] < cur_len:
                e = events[e_id]
                e["sent_id"] = i
                e["position"] = [e["position"][0]-old_len, e["position"][1]-old_len] 
    ...
    def populate_event_spans(self):
        ...
        event2id = {self.event_id(e):idx for idx, e in enumerate(events)} 

        self.label_groups = [[event2id[self.event_id(e)] for e in events] for events in merged_events] 
