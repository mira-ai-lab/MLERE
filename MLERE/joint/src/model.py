...
from transformers import AutoConfig, AutoModel, BertConfig, RobertaModel,AutoConfig
import torch
...
class EventEncoder(nn.Module):
    def __init__(self, vocab_size, model_name="", aggr="mean"):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load model configuration or model: {e}")
        ...
        for i in range(len(event_embed)):
            ...
            if embed.size(0) > 1: 
                ...
                split_scores = [to_cuda(torch.tensor([]))] \
                            + list(torch.split(s_ij, [i for i in range(len(embed)) if i], dim=0)) 
            else:
                split_scores = [to_cuda(torch.tensor([]))]
...
class Model(nn.Module):
    def __init__(self, vocab_size, model_name="", embed_dim=768, aggr="mean"):
        nn.Module.__init__(self)
